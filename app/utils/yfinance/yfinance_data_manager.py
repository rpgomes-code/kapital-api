...
def _sanitize_for_json(obj, depth=0, max_depth=10):
    ...
    try:
        ...
        # Special handling for yfinance's lazy-loading dict
        elif str(type(obj)).find('yfinance') != -1 and hasattr(obj, 'keys'):
            try:
                # Try to extract all keys from the lazy-loading dict
                regular_dict = {}
                for k in obj.keys():
                    try:
                        regular_dict[k] = obj[k]
                    except Exception:
                        regular_dict[k] = None
                return _sanitize_for_json(regular_dict, depth + 1, max_depth)
            except Exception as e:
                logger.debug(f"Failed to extract keys from yfinance object: {str(e)}")
                return str(obj)
